...
function MovieId() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const castData = await fetchMovies(
          BASE_URL,
          ENDPOINTS.MOVIE_CREDITS(id)
        );
        setCast(castData.cast);

        const reviewData = await fetchMovies(
          BASE_URL,
  ...
  const toggleFavorite = () => {
    ...
    const isCurrentlyFavorite = storedFavorites.some(
      (favId) => favId === movieId
    );
          ...
          <div className={css.additionalInfo}>
            ...
            <ul className={css.infoList}>
              <li className={css.infoItem}>
                <NavLink className={css.navLink} to={`/movies/${id}/cast`}>
                  Cast
                </NavLink>
              </li>
              <li className={css.infoItem}>
                <NavLink className={css.navLink} to={`/movies/${id}/reviews`}>
                  Reviews
                </NavLink>
              </li>
            </ul>
